...
export default function TifaAvatar({ size = "md", className = "", alt = "Tifa assistant avatar" }: TifaAvatarProps) {
  ...
  return (
    <span
      className={`inline-flex shrink-0 items-center justify-center overflow-hidden rounded-full border border-[var(--border-strong)] bg-[var(--bg-card)] shadow-[var(--avatar-shadow)] ${currentSize.outer} ${className}`}
      aria-label={alt}
