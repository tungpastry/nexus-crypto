...
- Deploy gotchas: the script pre-auths `sudo -v` early (fail fast) and refreshes (`sudo -n -v || sudo -v`) right before RESTART, because `npm ci`+build outlasts the 15-min `sudo` timestamp. The script injects `GIT_COMMIT_SHA/BUILD_TIME` into `.env.production.local`; a manual build+restart leaves the footer `VersionBadge` stale (it reads `/api/version` live, so updating the 4 keys + restart fixes it without rebuild).
