...
Next.js 16 + React 19 + TypeScript market-data dashboard (versioned Nexus Top 100 Universe). No trade execution, no custody, no financial advice copy.
...
- Source of truth for assets: generated `app/config/assets.generated.json`, exported by `app/config/assets.ts`. Current baseline is 100 assets: 52 Binance-enabled and 48 market-only. Stablecoins and Binance-unavailable assets use explicit market-only reasons; smoke/Tifa tests assert `analysis_enabled: false, mode: "market-only"`.
- Binance precision comes from committed `PRICE_FILTER.tickSize` metadata. The live panel is ticker price (5s); Decision Matrix `Kline Close` is candle price (60s), so small differences are expected.
...
- Provider health endpoints: `/api/provider-health` (light, representative check), `/deep` (8 core canaries, mount/manual only), `/{llm,ollama,gemini}` (`llm` is canonical for the active provider).
...
- Deploy gotchas: the script pre-auths `sudo -v` early (fail fast) and refreshes (`sudo -n -v || sudo -v`) right before RESTART, because `npm ci`+build outlasts the 15-min `sudo` timestamp. Agent sessions have no TTY and `sudo` timestamps don't cross into child processes — run agent-driven deploys via pty: `(for i in $(seq 1 10); do printf '%s\n' "$PASS"; done) | script -qec "bash scripts/deploy_ubuntu_server.sh" /dev/null`. The pty echoes the password into output: never persist pty logs, `shred` them immediately after the run.
- The script injects `GIT_COMMIT_SHA/BUILD_TIME` into `.env.production.local`; a manual build+restart leaves the footer `VersionBadge` stale (it reads `/api/version` live, so updating the 4 keys + restart fixes it without rebuild).

## Env & secrets

- Never commit or print `.env*` (gitignored: `.env`, `.env*.local`). Never `cat` `.env.production.local`; use `grep '^KEY='` for single values only.
- Optional live Gemini needs `GEMINI_API_KEY`, `GEMINI_MODEL=gemini-3-flash-preview`, `GEMINI_STREAM_ENABLED=1`, `GEMINI_CIRCUIT_BREAKER_ENABLED=1`. Production currently selects Ollama. No-secret-leak is contract-tested (`noSecretLeak.test.ts`, smoke `TIFA_PHASE2_NO_SECRET_LEAK`) — never interpolate keys/tokens into responses, SSE events, or logs.
...
- ESLint relaxations in `eslint.config.mjs`: `@typescript-eslint/no-explicit-any: off`, `react-hooks/set-state-in-effect: off`. The former Turbopack NFT tracing warning is resolved; treat its return as a regression and preserve the runtime-path ignore annotation.
- Branch `feature|fix|chore|docs/...`, commits `feat|fix|docs|chore(scope): ...`. Keep patches focused; preserve API/auth/deploy contracts.
- Start docs work at `docs/index.md`; canonical guides include `asset-catalog.md`, `nexus-algorithm.md`, and `tifa-assistant.md` in addition to API/auth/deploy/troubleshooting docs.
